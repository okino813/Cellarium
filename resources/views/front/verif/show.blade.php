...
@section('content')
    <div class="return-inter-page" style="padding: 15px;">
        ...
        <p class="instruction">Cochez les éléments présents dans chaque contenant</p>

        <div class="contenant-container">
            @foreach($contenants as $contenant)
                <div style="background-color: white; border-radius: 8px; padding: 10px;">
                    <h2 style="font-size: 20px; color: #2c3e50; border-bottom: 2px solid #B00020; padding-bottom: 10px;">
                        {{ $contenant->name }}
                    </h2>
                            ...
                            <div style="display:flex; gap:12px;">
                                ...
                                >
                                    <input
                                        ...
                                        "
                                    >
                                    <span class="item-text" style="text-align: left; flex: 1; color: #2c3e50; transition: all 0.3s ease;">
                                ...
                                <div id="overlay-{{ $item->id }}" style="display:none; position: fixed; top:0px; left: 0px; bottom: 0px; right:0px; background: rgba(76,76,76,0.8); justify-content: center; align-items: center">
                                    <form id="form-{{ $item->id }}" method="POST" style="display:flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; width:80%; background: white; padding: 50px; border: 2px solid black; border-radius: 20px;">
                                        ...
                                        <div class="end">
                                            <div class="quantity-display quantity-zero" id="display-{{ $item->id }}" style="width:60px; height: 40px; font-size: 25px">
                                                0
                                            </div>
        ...
        <form class="form-bottom" action="{{ route('front.verif.validate', $source->id ?? 1) }}" method="POST">
            @csrf
            <!-- Boutons d'action -->
            <button type="submit" class="btn" style="margin-bottom: 15px">
