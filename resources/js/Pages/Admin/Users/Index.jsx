import AppLayout from '@/Layouts/AppLayout'
import { Link } from '@inertiajs/react'

export default function Index({ users }) {
    console.log(users);

    const usersList = Array.isArray(users) ? users : Object.values(users);
    return (
        <div className="admin-page">
            <h1 className="title-user">Liste des utilisateurs</h1>
            <p className="instruction">Gérez tous les utilisateurs</p>

            <Link href="/admin/users/create" className="btn-add">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M256 64c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 160-160 0c-17.7 0-32 14.3-32 32s14.3 32 32 32l160 0 0 160c0 17.7 14.3 32 32 32s32-14.3 32-32l0-160 160 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-160 0 0-160z"/></svg>
            </Link>

            <div className="list-containing-admin">
                
                {usersList.length > 0 ? usersList.map(user => (
                    <Link key={user.id} className="card" href={`/admin/users/edit/${user.id}`}>
                            <p className="name">{user.firstname} {user.lastname}</p>
                            <p className="source">{user.matricule}</p>
                    </Link>
                )) : (
                    <div>
                        <p>Aucun utilisateur trouvé</p>
                        <Link href="/admin/users/create" style={{ color: '#007bff', textDecoration: 'underline' }}>
                            Ajouter votre premier utilsateur
                        </Link>
                    </div>
                )}
            </div>
        </div>
    )
}

Index.layout = page => <AppLayout>{page}</AppLayout>
