...
class AttributionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $containingWithItems = Containing::with('items')->where('firestation_id', $caserne->id)->get();

        // Récupère les Sources avec leurs Containing (si nécessaire)
        $sourcesWithContainings = Source::with('containings')->where('firestation_id', $caserne->id)->get();
    ...
    public function addItemContaining(Request $request)
    {
        ...
        $items = Item::where('firestation_id', $caserne->id)->get();
        $contenants = Containing::where('firestation_id', $caserne->id)->get();
    ...
    public function addItemContainingValidate(Request $request)
    {
        ...
        $containing = Containing::where('firestation_id', $caserne->id)->findOrFail($request->contenant);
        $item = Item::where('firestation_id', $caserne->id)->findOrFail($request->item);
    ...
    public function editItemContaining($containing_id, $item_id)
    {
        ...
        $containing = Containing::where("firestation_id", $caserne->id)->findOrFail($containing_id);
        $item = Item::where("firestation_id", $caserne->id)->findOrFail($item_id);
    ...
    public function ItemContainingUpdate(Request $request, $id)
    {
        ...
        $containing = Containing::where("firestation_id", $caserne->id)->findOrFail($id);
        $item = Item::where("firestation_id", $caserne->id)->findOrFail($request->item_id);
    ...
    public function addContainingSourceValidate(Request $request)
    {
        ...
        $containing = Containing::where("firestation_id", $caserne->id)->findOrFail($request->contenant);
        $source = Source::where("firestation_id", $caserne->id)->findOrFail($request->source);
    ...
    public function editContainingSource($containing_id, $source_id)
    {
        ...
        $containing = Containing::where("firestation_id", $caserne->id)->findOrFail($containing_id);
        $source = Source::where("firestation_id", $caserne->id)->findOrFail($source_id);
    ...
    public function ContainingSourceUpdate(Request $request, $id)
    {
        ...
        $containing = Containing::where("firestation_id", $caserne->id)->findOrFail($id);
        $source = Source::where("firestation_id", $caserne->id)->findOrFail($request->source_id);
